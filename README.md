...
Hideaway Candy Canes and Info — plus `/join`, which forwards to Scouting America's official
registration for the pack. Built with Vite and React, and prerendered to static HTML so
every page is a real URL.
...
site/
  src/
    ...
    routes.js    the route manifest — slug, title, description, og:image for every page
  ...
  public/
    ...
    hero/        the home-page hero still + the share-card still
```
...
interactive parts (the nav menu, the FAQ accordion). Search engines and
...
the Events page is meant to work, how joining works (`/join` forwards to Scouting America;
the interest form lives on `/info`), and the hosting setup. Start there before changing
content.
