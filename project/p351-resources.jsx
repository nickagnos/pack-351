// Resources page — tabbed layout

const FORMS = [
  { name: 'Annual Health & Medical Record (A & B)', note: 'Required for all scouts — complete annually', file: 'health-ab.pdf' },
  { name: 'Annual Health & Medical Record (C)',     note: 'Required for overnight events',               file: 'health-c.pdf' },
  { name: 'Activity Permission Slip',               note: 'Required per event — bring a signed copy',   file: 'permission.pdf' },
  { name: 'Talent Release (Photo permission)',       note: 'Optional — allows Pack to use photos',       file: 'talent.pdf' },
  { name: 'Scholarship Request',                    note: 'Confidential — no scout turned away',         file: 'scholarship.pdf' },
  { name: 'Pinewood Derby Car Rules',               note: 'Read before you start building!',             file: 'derby-rules.pdf' },
  { name: 'Raingutter Regatta Boat Rules',          note: 'Official construction guidelines',            file: 'regatta-rules.pdf' },
  { name: 'Campout Packing Checklist',              note: 'Printable checklist for families',            file: 'packing.pdf' },
];
    ...
    <div>
      ...
      <div style={{ background: 'var(--navy)', padding: '52px 0' }}>
        <div className="container">
          ...
          <p style={{ color: 'rgba(255,255,255,.72)', maxWidth: 520, lineHeight: 1.65, marginBottom: 28 }}>
          ...
          <div style={{
            ...
            background: '#fff', borderRadius: 8, padding: '10px 16px', maxWidth: 460,
          }}>
            <span style={{ fontSize: 17 }}>🔍</span>
            <input
              placeholder="Search forms, FAQs, uniform guide…"
              style={{ border: 'none', outline: 'none', fontFamily: 'Nunito', fontSize: 15, color: 'var(--text)', background: 'transparent', flex: 1 }}
            />
          </div>
        </div>
      </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
                {FORMS.map(f => (
                  <div key={f.name} className="card" style={{ padding: '16px 18px', display: 'flex', gap: 14, alignItems: 'center' }}>
                    ...
                    <button className="btn btn-ghost btn-sm" style={{ flexShrink: 0 }}>↓</button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <button className="btn btn-ghost btn-sm">Email cubmaster</button>
