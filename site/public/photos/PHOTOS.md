...
- **Resizing changes the pixel dimensions, and four of these files have theirs hardcoded
  ...
  downloads the image — so a stale number renders the preview at the wrong shape.
  **`npm run build` now fails if they disagree with the file**, naming the file, both sizes
  and the page, so you can't ship the mismatch — but the fix is still yours: update
  `routes.js` to match, then re-scrape in Facebook's Sharing Debugger. (`hero/trail.jpg`
  and `candy-cane-scene.png` carry the same coupling and the same check.)
