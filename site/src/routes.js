...
export const ROUTES = [
  ...
  },
  {
    ...
    description: 'Four ways to join Cub Scout Pack 351 in Lindale, TX: drop in to a Tuesday meeting, email us, leave your details on the interest form, or register online. Nothing to pay to come and look.',
...
// as its default, so EVERY link to /join gets the form anchor without the call site asking —
// including the ones generated by mapping over ROUTES/FOOTER_LINKS, where a per-link override
// would mean a `slug === 'join'` special case in each map. Linking to the top of a page here
// is still possible with an explicit empty hash: pageHref('join', '').
export const PAGE_ANCHORS = { join: JOIN_FORM_ID };
